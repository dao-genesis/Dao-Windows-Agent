...
test("route A 内嵌远程桌面全链路(白名单/前端按钮/宿主网关原语·非 mstsc)", () => {
  const out = applyPatches(fixture());
  // 免登录白名单纳入 winRdpEmbed
  assert.ok(NOAUTH_ADD.includes("winRdpEmbed"), "NOAUTH_ADD 缺 winRdpEmbed");
  // 前端: 内嵌连接按钮 + 状态 + iframe 挂载 + 消息回执
  assert.ok(out.includes("function wrGoEmbed("), "缺前端 wrGoEmbed");
  assert.ok(out.includes("function wrEmbClose("), "缺前端 wrEmbClose");
  assert.ok(out.includes("cmd('winRdpEmbed'"), "wrGoEmbed 未发 winRdpEmbed");
  assert.ok(out.includes("d.type==='winRdpEmbed'"), "缺 winRdpEmbed 回执处理");
  // 宿主: 网关原语 + case 分发
  assert.ok(out.includes("async function daoWinRdpEmbed("), "缺宿主 daoWinRdpEmbed");
  assert.ok(out.includes("case 'winRdpEmbed'"), "缺 winRdpEmbed case");
  // 内嵌走 view.html(单目标 canvas), 服务端持凭据不下发浏览器
  assert.ok(out.includes("view.html?ip="), "内嵌未指向 view.html 单目标视图");
