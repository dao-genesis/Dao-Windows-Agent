...
function handleControl(req, res) {
  ...
  // ★ ⑤ 内网穿透 · GET /origin/revproxy/bridge.md · 实时穿透配置文档 (Markdown)
  //   专注「帮用户/任意 Agent 配置内网穿透、对外提供反代底层 API」· 与整机穿透定位不同
  //   道法自然: 底层全开放 · 快速隧道 + 固定中继两通道并存 · 断线自愈 + 故障根治经验
  if (
    (u.pathname === "/origin/revproxy/bridge.md" ||
      u.pathname === "/origin/ea/bridge.md") &&
    req.method === "GET"
  ) {
    let md = "";
    try {
      ...
      md = "# dao-proxy-pro · 内网穿透文档生成失败\n\n" + (e && e.message);
    }
    res.setHeader("Content-Type", "text/markdown; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="dao-proxy-pro-bridge.md"',
    );
    res.end(md);
    return true;
  }
...
function _buildHandoffMd() {
  ...
  L.push("```");
  L.push("");
  L.push("### 帮用户配置一切 · 黄金配方 (Agent 照此逐步即可从零打通一个渠道)");
  L.push("");
  L.push("> 用户说「帮我配好某渠道 / 修好连不上」时, Agent 按此有序推进, 每步都即时生效·可验证:");
  L.push("");
  L.push("1. **看现状**: `GET $BASE/origin/ea/overview` — 现有渠道/路由/各渠道健康(`providers[x].health`)+不通原因。");
  L.push("2. **加渠道**: `POST $BASE/origin/ea/provider` 填 `{name, config:{enabled:true, apiKey, baseUrl, type:\"openai-compatible\", protocol:\"openai-chat\"}}`(详见「三·热配渠道」)。");
  L.push("3. **发现模型**: `GET $BASE/origin/ea/discover-models?provider=NAME` 自动拉该渠道 `/v1/models`, 确认可用模型名。");
  L.push("4. **连路由**: `POST $BASE/origin/ea/route` 把官方模型 UID 映到该渠道模型(详见「三·热配路由」; 官方 UID 见「二」表)。");
  L.push("5. **验证**: `POST $BASE/origin/ea/probe`(全渠道实证探活) + `POST $BASE/origin/ea/test-chat {modelUid,message}`(端到端冒烟)。坏渠道会如实报错因。");
  L.push("6. **对外(可选)**: 需把已通模型开放到公网供其他工具/设备直调, 转「⑤ 内网穿透」——读 `GET $BASE/origin/revproxy/bridge.md`(专文, 含隧道/固定中继/故障根治)。");
  L.push("");
  L.push("> 两份实时文档分工: **本文档(handoff.md)** = 渠道/路由/提示词热配; **bridge.md** = 内网穿透/反代对外通道。二者同源实时生成。");
  L.push("");
...
// ★ ⑤ 内网穿透 · 实时穿透配置文档 (Markdown) · 供用户/任意 Agent 配置反代底层 API 公网通道
//   道义: 与「操作整机」定位不同 · 专注帮用户配置内网穿透、对外提供反代底层 API。
//   两条通道并存(道并行而不相悖): 默认快速隧道(零账号·轮换) + 可选固定中继(一个 CF Token·永不轮换)。
function _buildBridgeMd() {
  ...
  const now = new Date().toISOString();
  const keyShow = rpKey || "$REVPROXY_KEY";
  const L = [];
  L.push("# ☯ dao-proxy-pro · ⑤ 内网穿透 · 反代底层 API 公网通道配置文档");
  L.push("");
  L.push("> 道法自然 · 无为而无不为。本文件帮**用户/任意 Agent** 配置本插件第五模块「内网穿透」——");
  L.push("> 把「④ 模型反代」已接通的免费/付费模型经**反代底层 API**(`/v1/*`、`/origin/revproxy/*`)");
  L.push("> 暴露到**公网**, 任意 AI 工具凭 `公网Base + 同一 apiKey` 直调。与「操作整机」定位不同——");
  L.push("> 这里**专注对外提供反代底层 API**。把本文件交给运行中的任意 Agent, 即可照此热配置隧道·无需重启。");
  L.push("");
  L.push("- 生成时间: `" + now + "`");
  L.push("- 控制面 Base URL: `" + base + "` (仅本机 127.0.0.1 · 无需鉴权)");
  L.push("- 端点发现文件: `" + (function () { const d = _daoUserDir(); return d ? path.join(d, "endpoint.json") : "(不可用)"; })() + "` (含 `revproxy.tunnel` 公网端点自省, 隧道轮换后先读此文件)");
  L.push("");
  L.push("## 一、当前状态 (实时快照)");
  L.push("");
  L.push("| 项 | 值 |");
  L.push("| --- | --- |");
  L.push("| 反代数据面 (本机) | `" + base + "/v1` |");
  L.push("| 反代启用 | " + (rpEnabled === null ? "未知" : rpEnabled ? "✓ 已启用" : "✗ 未启用(先开启, 见四)") + " |");
  L.push("| API Key | " + (rpKey ? "`" + rpKey + "` (仅本机文档可见)" : "未设置(启动隧道自动生成, 或见四)") + " |");
  if (b) {
    L.push("| 快速隧道 | " + (b.running && b.url ? "✓ 已连通" + (b.shared ? "(复用共享隧道)" : b.named ? "(命名隧道)" : "(快速隧道)") : b.connecting ? "连接中…" : "✗ 未连通") + " |");
    if (b.url) L.push("| 公网 Base URL | `" + b.publicBase + "` |");
    L.push("| 固定中继 (workers.dev) | " + (b.relay && b.relay.bound ? (b.relay.connected ? "✓ 已上线(永不轮换)" : "已绑定·连接中") + (b.relay.publicUrl ? " · `" + b.relay.publicUrl + "`" : "") : "未绑定(可选, 见二-B)") + " |");
  }
  L.push("");
  L.push("## 二、两条公网通道 (道并行而不相悖 · 按需取一)");
  L.push("");
  L.push("### A. 快速隧道 (默认 · 零账号 · 即开即用 · URL 随重启轮换)");
  L.push("cloudflared quick tunnel, 无需任何账号/域名, 即开即用。**代价**: 每次重启换新 URL。");
  L.push("```bash");
  L.push("# 启动 (等价于 ⑤ 面板「▶ 启动隧道」)");
  L.push("curl -X POST " + base + "/origin/revproxy/tunnel -H 'Content-Type: application/json' -d '{\"action\":\"start\"}'");
  L.push("# 换新 URL / 停止");
  L.push("curl -X POST " + base + "/origin/revproxy/tunnel -H 'Content-Type: application/json' -d '{\"action\":\"restart\"}'");
  L.push("curl -X POST " + base + "/origin/revproxy/tunnel -H 'Content-Type: application/json' -d '{\"action\":\"stop\"}'");
  L.push("```");
  L.push("");
  L.push("### B. 固定中继 (可选 · 一个 Cloudflare API Token · 零域名 · 永不轮换 · 持久化自愈) — **首选**");
  L.push("在 [Cloudflare 面板](https://dash.cloudflare.com/profile/api-tokens) 建一个 API Token(权限含 `Account · Workers Scripts:Edit`),");
  L.push("系统自动把最小中继 Worker 部署到**你自己**账号的免费 `*.workers.dev` 子域, 得到**永不轮换**的公网入口(重启自愈)。");
  L.push("```bash");
  L.push("# 绑定并固定 (等价于 ⑤ 面板「绑定并固定」)");
  L.push("curl -X POST " + base + "/origin/revproxy/tunnel -H 'Content-Type: application/json' -d '{\"action\":\"bindCf\",\"token\":\"<CF_API_TOKEN>\"}'");
  L.push("# 启停中继 / 干净解绑(即使数据损坏也可清后重绑)");
  L.push("curl -X POST " + base + "/origin/revproxy/tunnel -H 'Content-Type: application/json' -d '{\"action\":\"relayStart\"}'");
  L.push("curl -X POST " + base + "/origin/revproxy/tunnel -H 'Content-Type: application/json' -d '{\"action\":\"logout\"}'");
  L.push("```");
  L.push("> ⚠ **DNS 污染根治经验**: `*.workers.dev` 在中国大陆常被 DNS 污染(解析到非 Cloudflare IP → 长连超时/530)。");
  L.push("> 若 workers.dev 中继长连不上而快速隧道正常, 十有八九是这个。**根治**: 若你有自有域名, 在 CF 给中继 Worker");
  L.push("> 绑一个 **Custom Domain**(如 `relay.你的域名`), 解析走真 Cloudflare IP 即绕过污染; 无域名则以快速隧道兜底。");
  L.push("");
  L.push("## 三、公网直调 (标准 SDK · 即调即用)");
  L.push("");
  const pubBase = b && b.url ? b.publicBase : "<公网Base · 启动隧道后取>";
  const pubChat = b && b.url ? b.publicChat : "<公网Base>/v1/chat/completions";
  L.push("公网侧把 Base URL 换成**公网URL**、Header 仍带**同一 apiKey** 即可(model 可填 modelUid 或家族别名如 `glm-4.7`):");
  L.push("```bash");
  L.push("# OpenAI 兼容 · 对话补全");
  L.push("curl -X POST " + pubChat + " \\");
  L.push("  -H 'Content-Type: application/json' -H 'Authorization: Bearer " + keyShow + "' \\");
  L.push("  -d '" + JSON.stringify({ model: "swe-1-6", messages: [{ role: "user", content: "你好" }], stream: false }) + "'");
  L.push("# 列出可反带模型 · Claude(Anthropic) 兼容端点为 " + pubBase + "/messages");
  L.push("curl -s " + pubBase + "/models -H 'Authorization: Bearer " + keyShow + "'");
  L.push("```");
  L.push("> 固定中继(B)公网侧调用格式(relay session+token 即凭证, 反代 apiKey 由本机自动注入):");
  L.push("> `POST <中继公网入口> -H 'Authorization: Bearer <relayToken>' -d '{\"path\":\"/v1/chat/completions\",\"method\":\"POST\",\"body\":{...}}'`");
  L.push("");
  L.push("## 四、Agent 热配 API (即时生效 · 不重启)");
  L.push("");
  L.push("- `GET  /origin/revproxy/status` — 反代状态 + apiKey + 全模型(含 `exposed` 外接标记) + 家族");
  L.push("- `POST /origin/revproxy/config` — 热配 `{enabled,exposeLan,defaultMaxTokens,disabledModels,regenerateKey}`(开启反代/换 key/选择性反带)");
  L.push("- `GET  /origin/revproxy/tunnel` — 隧道+中继实时状态 · `POST` 同端点带 `{action}` 管理隧道(见二)");
  L.push("- `POST /origin/revproxy/models` — 选择性反带 `{modelUids:[...],exposed:true|false}` 或 `{setAll:\"on|off\"}`");
  L.push("```bash");
  L.push("# 开启反代 (若未启用) + 确保有 key");
  L.push("curl -X POST " + base + "/origin/revproxy/config -H 'Content-Type: application/json' -d '{\"enabled\":true}'");
  L.push("# 重新生成 API Key");
  L.push("curl -X POST " + base + "/origin/revproxy/config -H 'Content-Type: application/json' -d '{\"regenerateKey\":true}'");
  L.push("```");
  L.push("");
  L.push("## 五、断线自愈 (URL 轮换零人工)");
  L.push("");
  L.push("1. **首选固定中继(B)**: 永不轮换, 一次绑定长期有效, 重启自愈, 无需重发地址。");
  L.push("2. 若用快速隧道(A): 公网 URL 会随重启轮换。遇旧 URL 打不通——**先读发现文件 `endpoint.json`** 的");
  L.push("   `revproxy.tunnel.public_base`(本机落盘·最准), 或 `GET " + base + "/origin/revproxy/tunnel` 拿当前 URL, 不必找用户重发。");
  L.push("");
  L.push("## 六、故障排查 (根治经验 · 名实相符)");
  L.push("");
  L.push("| 症状 | 根因 | 根治 |");
  L.push("| --- | --- | --- |");
  L.push("| 公网 530 / 长连秒断 | cloudflared 到本地 :" + _actualPort + " 的连接被切(杀软/防火墙 TLS 深检或本地代理切长连) | 重启隧道(`restart`); 排查本机代理/杀软对长连的拦截; 优先改用固定中继(B) |");
  L.push("| workers.dev 中继连不上 | `*.workers.dev` DNS 污染(解析到非 CF IP) | 有域名则给中继 Worker 绑 Custom Domain 绕过; 无则退回快速隧道 |");
  L.push("| 公网 401 / 鉴权失败 | 缺 `Authorization: Bearer <apiKey>` 或 key 不符 | `GET /origin/revproxy/status` 取当前 apiKey; 公网 Header 带同一 key |");
  L.push("| `/v1/models` 空 / 调用 403 model_not_exposed | 反代未启用 或 该模型被排除外接 | `config {enabled:true}`; `models {modelUids:[...],exposed:true}` |");
  L.push("| 隧道启动失败·找不到 cloudflared | 二进制缺失/被代理挡下载 | 面板会自动 7 端口探代理+6 路镜像回退下载·断点续传; 仍失败则手装 cloudflared 到 PATH |");
  L.push("");
  L.push("---");
  L.push("_本文件由 " + base + "/origin/revproxy/bridge.md 实时生成 · 反映内网穿透模块当前真实状态 · " + _extVersion() + "_");
  return L.join("\n");
}
...
async function _maybeRevproxy(req, res) {
  if (
    ...
    return false;
  // ★ ⑤ bridge.md 是实时配置文档(GET·永远可读, 不依赖反代是否启用) → 让给 ea handler
  if (req.url.split("?")[0] === "/origin/revproxy/bridge.md") return false;
  const mod = _getRevproxy();
  if (!mod) return false;
  const u = url.parse(req.url, true);
