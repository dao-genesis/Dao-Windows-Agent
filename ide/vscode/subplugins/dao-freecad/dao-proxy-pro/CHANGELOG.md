...
v9.9.345 · ⑤ 内网穿透 专属配置文档 + 渠道配置交接文档强化
: 新增实时生成的**内网穿透配置文档** `GET /origin/revproxy/bridge.md`——专注帮用户/任意
  Agent 配置反代底层 API 公网通道: 当前状态实时快照、两条通道(零账号快速隧道 vs 一个 CF Token
  的 workers.dev 固定中继·永不轮换)、公网直调标准 SDK、Agent 热配 API、URL 轮换断线自愈、
  以及**故障根治经验表**(530/长连被切、workers.dev 在中国大陆 DNS 污染→Custom Domain 绕过、
  401/model_not_exposed 等)。⑤ 面板底部加「📋 复制 / ⬇ 下载 / 👁 预览 配置文档」三键(宿主
  存盘 `saveBridgeDoc`),与 ④ 渠道配置的交接文档同构。handoff.md 零章新增「帮用户配置一切·
  黄金配方」6 步(看现状→加渠道→发现模型→连路由→验证→对外),并与 bridge.md 分工互链。
