...
<body data-port="${proxyPort}">
  ...
  <div class="dao-pane" id="paneBridge">
    ...
    <!-- ★ 内网穿透配置文档 (实时生成 · 帮用户/任意 Agent 配置反代底层 API 公网通道) -->
    <div style="border:1px solid rgba(88,166,255,0.28);border-radius:6px;padding:8px;margin:8px 2px 4px">
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        <span style="font-weight:600;font-size:11px">📄 内网穿透配置文档</span>
        <span style="font-size:10px;opacity:0.6">交给任意 Agent 即可照此配置穿透/反代/隧道·热管一切</span>
        <button class="btn add" id="brgCopyDoc" style="margin-left:auto" title="一键复制内网穿透配置文档到剪贴板 · 直接粘给本地任意 Agent 即可接管配置内网穿透">📋 复制配置文档</button>
        <button class="btn" id="brgDownloadDoc" title="下载 dao-proxy-pro-bridge.md">⬇ 下载 MD</button>
        <button class="btn" id="brgPreviewDoc" title="预览内网穿透配置文档">👁 预览</button>
      </div>
      <pre id="brgDocPreview" style="display:none;max-height:220px;overflow:auto;margin:6px 0 0;padding:8px;font-size:10px;line-height:1.45;white-space:pre-wrap;word-break:break-word;background:var(--vscode-textCodeBlock-background,rgba(0,0,0,0.18));border-radius:4px"></pre>
    </div>
  </div>
...
(function() {
  ...
  // ═══ ⑤ 内网穿透配置文档 · 实时生成 · 复制 / 下载 / 预览 ═══
  function _fetchBridgeDoc() {
    return fetch(_BASE + '/origin/revproxy/bridge.md', { cache: 'no-store' })
      .then(function(r) { if (!r.ok) throw new Error('http ' + r.status); return r.text(); });
  }
  var _bcd = _e1El('brgCopyDoc');
  if (_bcd) _bcd.addEventListener('click', function() {
    var self = this; var _orig = self.textContent; self.textContent = '取最新…';
    _fetchBridgeDoc().then(function(md) {
      function _viaHost() { if (_vscode) _vscode.postMessage({ type: 'copyHandoff', content: md }); }
      var done = function() { self.textContent = '✓ 已复制'; setTimeout(function() { self.textContent = _orig; }, 1800); };
      try {
        if (navigator.clipboard && navigator.clipboard.writeText) {
  ...
  var _bdd = _e1El('brgDownloadDoc'), _bpd = _e1El('brgPreviewDoc');
  if (_bdd) _bdd.addEventListener('click', function() {
    _fetchBridgeDoc().then(function(md) {
      if (_vscode) { _vscode.postMessage({ type: 'saveBridgeDoc', content: md }); return; }
      try {
        var blob = new Blob([md], { type: 'text/markdown' });
        var url = URL.createObjectURL(blob);
        var a = document.createElement('a');
        a.href = url; a.download = 'dao-proxy-pro-bridge.md';
        document.body.appendChild(a); a.click();
        setTimeout(function() { URL.revokeObjectURL(url); a.remove(); }, 1000);
      } catch (e2) {}
    }).catch(function(e) { try { _daoToast('下载失败: ' + e.message); } catch (_) {} });
  });
  if (_bpd) _bpd.addEventListener('click', function() {
    var pre = _e1El('brgDocPreview');
    _fetchBridgeDoc().then(function(md) { if (pre) { pre.textContent = md; pre.style.display = 'block'; } })
      .catch(function(e) { if (pre) { pre.textContent = '预览失败: ' + e.message; pre.style.display = 'block'; } });
  });
...
// ★ ⑤ 内网穿透配置文档 · webview 下载按钮 → 宿主存盘
async function _saveBridgeDoc(content) {
  try {
    const def = vscode.Uri.file(
      require("path").join(
        require("os").homedir(),
        "dao-proxy-pro-bridge.md",
      ),
    );
    const uri = await vscode.window.showSaveDialog({
      defaultUri: def,
      filters: { Markdown: ["md"] },
      saveLabel: "保存内网穿透配置文档",
    });
    if (!uri) return;
    await vscode.workspace.fs.writeFile(
      uri,
    ...
    vscode.window.showInformationMessage(
      `内网穿透配置文档已保存: ${uri.fsPath}`,
    );
  } catch (e) {
    vscode.window.showErrorMessage(`内网穿透配置文档保存失败: ${e && e.message}`);
  }
}
  ...
  resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage((msg) => {
      try {
        ...
        else if (msg.type === "saveBridgeDoc") _saveBridgeDoc(msg.content || "");
  ...
  try {
    ...
    panel.webview.onDidReceiveMessage((msg) => {
      try {
        ...
        } else if (msg.type === "saveBridgeDoc") {
          _saveBridgeDoc(msg.content || "");
