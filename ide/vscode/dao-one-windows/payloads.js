...
"var WMOD='config';",
"/* 面板内嵌桌面态: 一切操作不出 VS Code 面板 —— 连接即在本板块内整块渲染桌面(独立页仅作可选)。 */",
"var WDESK={on:false,url:null,msg:null,label:'',account:''};",
"var WDESKMODE='embed';",
...
"  var keep=document.getElementById('wkeep');",
"  if(!keep){keep=document.createElement('div');keep.id='wkeep';keep.style.display='none';v.appendChild(keep);}",
"  var sw='<div class=\"st\">🪟 Windows · 远程桌面连接(官方全功能收编 · 连接即面板内直显桌面)</div>';",
"  sw+='<div class=\"card\"><div class=\"cr\"><span class=\"v\" style=\"display:flex;gap:6px;flex-wrap:wrap\">'+",
"    '<button class=\"btn'+(WMOD==='config'?'':' ghost')+'\" onclick=\"wmSwitch(&#39;config&#39;)\">① 远程桌面连接 · 配置台</button>'+",
"    '<button class=\"btn'+(WMOD==='pool'?'':' ghost')+'\" onclick=\"wmSwitch(&#39;pool&#39;)\">② 账号池 · 多账号管理</button>'+",
"    (WDESK.on?('<button class=\"btn'+(WMOD==='desk'?'':' ghost')+'\" onclick=\"wmSwitch(&#39;desk&#39;)\">🖥 '+esc(WDESK.label||WDESK.account)+'</button>'):'')+",
"    '</span></div></div>';",
"  if(WMOD==='desk'){rWinDesk(main,sw);return;}",
"  keep.style.display='none';",
"  if(WMOD==='pool'){rWinPool(main,sw);return;}",
"  rWinConfig(main,sw);}",
"function wmSwitch(m){WMOD=(m==='pool'?'pool':(m==='desk'?'desk':'config'));rWindows();}",
...
"var WDESKOPTS={};",
"function wdeskOpen(account,label,target,opts,mode){if(!account){toast('缺账号/连接名',false);return;}",
"  WDESKMODE=(mode==='page'?'page':'embed');",
"  WDESKOPTS[account]=opts||null;",
"  toast((WDESKMODE==='page'?'打开独立桌面页':'面板内连接远程桌面')+' · '+(label||account),true);cmd('winDeskEnsure',{account:account,label:(label||account),target:target||null});}",
"/* .rdp 档案键 → 官方 token 会话选项 —— 官方五页(显示/本地资源/体验)全量直通隙道 /token:",
"   色深/壁纸/视觉样式/字体平滑/拖拉/动画/合成/位图缓存/音频/录音/打印/剪贴板/驱动器,",
"   面板内 Guacamole 桌面 = 官方 mstsc 同义配置, 只搬运不生产。 */",
"function wdeskOptsQ(o){if(!o)return '';var q='';",
"  function tb(v,d){return (v===undefined||v===null)?d:!!v;}",
"  if(o.clipboard===false)q+='&clipboard=off';",
"  if(o.drives)q+='&drive='+encodeURIComponent('C:\\\\dao-share');",
"  if(o.readonly)q+='&readonly=1';",
"  var bpp=parseInt(o.bpp,10);if(bpp===8||bpp===16||bpp===24||bpp===32)q+='&colordepth='+bpp;",
"  if(o.wallpaper!==undefined)q+='&wallpaper='+(tb(o.wallpaper,true)?'1':'0');",
"  if(o.themes!==undefined)q+='&theming='+(tb(o.themes,true)?'1':'0');",
"  if(o.fontsmoothing!==undefined)q+='&fontsmoothing='+(tb(o.fontsmoothing,false)?'1':'0');",
"  if(o.fullwindowdrag!==undefined)q+='&windowdrag='+(tb(o.fullwindowdrag,false)?'1':'0');",
"  if(o.composition!==undefined)q+='&composition='+(tb(o.composition,false)?'1':'0');",
"  if(o.menuanims!==undefined)q+='&animations='+(tb(o.menuanims,false)?'1':'0');",
"  if(o.bitmapcache!==undefined)q+='&bitmapcache='+(tb(o.bitmapcache,true)?'1':'0');",
"  var am=parseInt(o.audiomode,10);",
"  if(am===2)q+='&audio=off';",
"  else if(parseInt(o.audiocapture,10)===1)q+='&audio=both';",
"  else if(am===0||am===1)q+='&audio=out';",
"  if(o.printers!==undefined)q+='&printing='+(tb(o.printers,false)?'1':'0');",
"  return q;}",
...
"  /* 本源: 默认面板内嵌——桌面就在 VS Code 归一面板本板块内整块渲染; 独立页仅作显式可选。 */",
"  if(WDESKMODE!=='page'){WDESK={on:true,url:u,msg:msg,label:lbl,account:d.account};WMOD='desk';rWindows();return;}",
...
"/* 面板内桌面视图: 工具条(返回/弹独立页) + 整块 Guacamole 桌面 iframe。",
"   iframe 常驻 #wkeep(切页仅隐藏不重建), 免得每次切回都重载桌面页而掉线重连。 */",
"function rWinDesk(v,h){if(!WDESK.on||!WDESK.url){WMOD='config';rWindows();return;}",
"  h+='<div class=\"card\"><div class=\"cr\"><span class=\"l\">🖥 '+esc(WDESK.label||WDESK.account)+' · 远程桌面(面板内)</span><span class=\"v\">'+",
"    '<button class=\"btn ghost\" onclick=\"wdeskBack()\">‹ 返回管理面</button> '+",
"    '<button class=\"btn ghost\" onclick=\"wdeskPop()\">独立页↗</button></span></div></div>';",
"  v.innerHTML=h;",
"  var keep=document.getElementById('wkeep');if(!keep)return;",
"  keep.style.display='';",
"  var f=document.getElementById('wdesk_frame');",
"  if(!f||f.getAttribute('data-src')!==WDESK.url){",
"    keep.innerHTML='<iframe id=\"wdesk_frame\" data-src=\"'+esc(WDESK.url)+'\" src=\"'+esc(WDESK.url)+'\" style=\"width:100%;height:calc(100vh - 210px);min-height:420px;border:1px solid var(--line);border-radius:8px;background:#000\" allow=\"clipboard-read; clipboard-write\"></iframe>';}}",
"function wdeskBack(){WMOD='config';rWindows();}",
"function wdeskPop(){if(!WDESK.msg)return;var msg=WDESK.msg;",
"  try{if(window.parent&&window.parent!==window){window.parent.postMessage(msg,'*');}",
"    else if(window.top&&window.top!==window){window.top.postMessage(msg,'*');}",
"    else{cmd('winDeskOpenExternal',{url:msg.url});}}catch(e){cmd('winDeskOpenExternal',{url:msg.url});}}",
...
"  /* 官方「远程桌面连接」同款入口: 计算机 + 用户名 + 连接 —— 一眼即会, 连接即面板内直显桌面。 */",
"  h+='<div class=\"card\"><div class=\"cr\"><span class=\"l\"><b>🖥 远程桌面连接</b></span><span class=\"v\" style=\"color:var(--muted)\">同官方 mstsc 对话框 · 输入计算机名即连</span></div>'+",
"    '<div class=\"cr\"><span class=\"l\">计算机(C)</span><span class=\"v\"><input id=\"wq_host\" placeholder=\"示例: 127.0.0.1 或 computer.fabrikam.com\" value=\"127.0.0.1\"> : <input id=\"wq_port\" size=\"5\" value=\"3389\"></span></div>'+",
"    '<div class=\"cr\"><span class=\"l\">用户名</span><span class=\"v\"><input id=\"wq_user\" placeholder=\"留空则连当前控制台\"> <button class=\"btn primary\" onclick=\"wqConnect()\">连接(N)</button> <button class=\"btn ghost\" onclick=\"wrNew()\">显示选项(O) · 五页配置</button></span></div></div>';",
"  h+='<div class=\"card\"><div class=\"cr\"><span class=\"l\">保存的连接档案 '+WRD.list.length+' 个 · 官方五页全功能收编</span><span class=\"v\"><button class=\"btn primary\" onclick=\"wrNew()\">＋新建</button> <button class=\"btn ghost\" onclick=\"cmd(&#39;winRdpOpenDir&#39;)\">打开目录</button> <button class=\"btn ghost\" onclick=\"wrReload()\">⟳</button></span></div></div>';",
"  for(var i=0;i<WRD.list.length;i++){var p=WRD.list[i];",
"    h+='<div class=\"card\"><div class=\"cr\"><span class=\"l\">🖥 '+esc(p.name||'')+'</span><span class=\"v\">'+",
"      '<button class=\"btn primary\" onclick=\"wrOpenDesk('+i+')\">连接</button> '+",
"      '<button class=\"btn ghost\" onclick=\"wrOpenDeskPage('+i+')\">独立页↗</button> '+",
...
"function wrOpenDesk(i){var p=WRD.list[i];if(!p)return;wdeskOpen(p.name,p.name,{hostname:(p.host||'127.0.0.1'),port:(p.port||'3389'),username:(p.username||'')},p);}",
"function wrOpenDeskPage(i){var p=WRD.list[i];if(!p)return;wdeskOpen(p.name,p.name,{hostname:(p.host||'127.0.0.1'),port:(p.port||'3389'),username:(p.username||'')},p,'page');}",
"function wqConnect(){var g=function(id){return document.getElementById(id);};var host=(g('wq_host').value||'').trim()||'127.0.0.1';var port=(g('wq_port').value||'').trim()||'3389';var user=(g('wq_user').value||'').trim();",
"  var name='快速连接·'+host;wdeskOpen(name,host+(user?(' · '+user):''),{hostname:host,port:port,username:user});}",
...
"      '<button class=\"btn primary\" onclick=\"waOpenDesk('+i+')\">连接</button> '+",
"      '<button class=\"btn ghost\" onclick=\"waOpenDeskPage('+i+')\">独立页↗</button> '+",
...
"function waOpenDeskPage(i){var a=WACC.list[i];if(!a)return;var t=a.target||{};wdeskOpen(a.name,a.name,{hostname:(t.hostname||'127.0.0.1'),port:(t.port||'3389'),username:a.name},null,'page');}",
