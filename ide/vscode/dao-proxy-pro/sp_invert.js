...
// ── 经文加载 ──────────────────────────────────────────────
const _BUNDLED_DIR = path.join(__dirname, "bundled-origin");
...
function _currentModeState() {
  const now = Date.now();
  if (now - _lastModeCheck < 500) return _modeState;
  _lastModeCheck = now;
  try {
    const raw = fs.readFileSync(MODE_STATE_PATH, "utf8");
    const data = JSON.parse(raw);
    _modeState =
      data && typeof data.mode === "string"
        ? { mode: data.mode, overlay: typeof data.overlay === "string" ? data.overlay : "" }
        : null;
  } catch {
    _modeState = null;
  }
...
function invertAnySP(spText) {
  try {
    ...
    if (_modeSkipsInvert()) return null;
    const keeps = extractKeepBlocks(s);
    _maybeHotReloadToolMode();
    const base = _canonHeader(_activeCanon) + _activeCanonText + _toolContractSuffix() + TAO_FOOTER + _modeOverlaySuffix();
...
module.exports = {
  ...
  // 状态查询
  getMode: () => SP_MODE,
  getModeState: () => _currentModeState(),
