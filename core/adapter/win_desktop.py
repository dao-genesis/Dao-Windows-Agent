...
if _IS_WIN:  # pragma: no cover - 仅 Windows guest 内执行
    ...
    def snapshot_class_windows(name: str, class_name: str) -> set:
        """记录某桌面上指定类名的顶层窗口集合（供比对"新窗口"用）。"""
        return {h for h, _t in enum_windows(name, visible_only=False)
                if _class_name(h) == class_name}

    def find_new_class_window(name: str, class_name: str, exclude: set) -> Optional[int]:
        """在某桌面找 exclude 之外最新出现的指定类名顶层窗口；没有则返回 None。"""
        for h, _t in reversed(enum_windows(name, visible_only=False)):
            if h not in exclude and _class_name(h) == class_name:
                return h
        return None
    ...
    def window_rect(hwnd: int) -> Tuple[int, int, int, int]:
        """返回顶层窗口的 (left, top, width, height)；无效窗口返回全 0。"""
        rect = wintypes.RECT()
        if not _user32.GetWindowRect(hwnd, ctypes.byref(rect)):
            return (0, 0, 0, 0)
        return (rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top)

    def find_main_window(name: str, title_contains: Optional[str] = None,
                         class_name: Optional[str] = None,
                         min_area: int = 40_000) -> Optional[int]:
        """在隔离桌面上挑**真正的主窗口**：可见、面积最大且过 min_area 的匹配顶层窗口。

        实机之痛（FreeCAD 等富客户端）：首启会并存启动闪屏、"Invalid OpenGL" 告警框、
        0×0 幽灵顶层窗口，标题都含应用名——`find_top_window` 只按标题子串取最后一个，
        极易命中幽灵/对话框，导致截图取证与消息级操作打空。以「可见 + 面积最大」择主窗，
        与人眼所见的主界面一致，对多窗口应用稳健（min_area 滤除对话框/闪屏/0 尺寸幽灵）。
        """
        best: Optional[int] = None
        best_area = -1
        for h, title in enum_windows(name, visible_only=True):
            if title_contains and title_contains.lower() not in title.lower():
                continue
            if class_name and _class_name(h) != class_name:
                continue
            _l, _t, w, ht = window_rect(h)
            area = w * ht
    ...
    def capture_window(hwnd: int, path: str) -> str:
        ...
        if not _user32.IsWindow(hwnd):
            raise OSError(f"窗口 {hwnd:#x} 已销毁，无法截图（富客户端首启期 hwnd 常被重建，"
                          "应在定位到主窗后立即取证，勿在长等待后再截）")
        rect = wintypes.RECT()
        _user32.GetWindowRect(hwnd, ctypes.byref(rect))
        w = rect.right - rect.left
        h = rect.bottom - rect.top
        if w <= 0 or h <= 0:
            raise OSError(f"窗口 {hwnd:#x} 尺寸无效({w}x{h})，无法截图")
    ...
    is_window = _unavailable  # type: ignore[assignment]
    snapshot_class_windows = _unavailable  # type: ignore[assignment]
    find_new_class_window = _unavailable  # type: ignore[assignment]
    find_top_window = _unavailable  # type: ignore[assignment]
    window_rect = _unavailable  # type: ignore[assignment]
    find_main_window = _unavailable  # type: ignore[assignment]
